...
describes what users get. Pick a format with `-o`, or let galaxyplay choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
galaxyplay <command> -o table   # aligned columns for reading
galaxyplay <command> -o jsonl   # one JSON object per line, for piping
galaxyplay <command> -o json    # a single JSON array
galaxyplay <command> -o csv     # spreadsheet friendly
galaxyplay <command> -o tsv     # tab-separated
galaxyplay <command> -o url     # just the URL column
galaxyplay <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
galaxyplay <command> --fields id,title,url
```
...
```bash
galaxyplay <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
galaxyplay <command>            # a table, because this is a terminal
galaxyplay <command> | wc -l    # JSONL, because this is a pipe
```
